از تمپلیت داده‌شده برای پیاده‌سازی استفاده شده است. برای اجرا، کافی‌ست از دستور زیر استفاده شود:
```bash
make run
```
توابع موردنظر در فایل `scheduler.c` پیاده‌سازی شده‌اند؛ و برای برخی قسمت‌های کد، کامنت نیز قرار داده شد.
### خروجی کد
```
==== Test Case 1 ====
